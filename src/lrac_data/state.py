...
class FileIdentity:
    ...
    def from_dict(cls, value: object) -> FileIdentity | None:
        if not isinstance(value, Mapping):
            return None
        fields = tuple(
            value.get(name) for name in ("size", "mtime_ns", "ctime_ns", "device", "inode")
        )
        if not all(type(field) is int for field in fields):
            return None
        return cls(*cast(tuple[int, int, int, int, int], fields))
...
class StateStore:
    ...
    def is_complete(
        ...
        *,
        ...
        state = self.read(key)
        if state is None or state.status != "complete" or state.fingerprint != expected_fingerprint:
            return False
...
def environment_provenance(repo_root: Path) -> dict[str, Any]:
    """Collect tool and package identities that affect preparation output."""
    ...
    packages["libsndfile"] = _libsndfile_version()
...
def _libsndfile_version() -> str | None:
    try:
        from soundfile import __libsndfile_version__
    except ImportError:
        return None
    return str(__libsndfile_version__)
